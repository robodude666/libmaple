...
/*
 * The ADC DMA channel will take ADC_BUF_SIZE samples.
...
volatile bool doing_uart_transfer = false;
volatile bool dma_error_occured = false;
...
void loop(void) {
    if (dma_error_occured) {
        /*
         * An error occured. Do whatever we want here like try the DMA transfer
         * again. For this demo, well blink the LED extra fast. The STM32
         * disables the DMA controller on our behalf, so you'll have to
         * enable it again if you want to attempt another transfer.
         */
...
void init_dma_adc(void) {
    ...
    dma_setup_transfer(ADC_DMA_DEV, ADC_DMA_CHANNEL,
                        ...
                        (DMA_MINC_MODE | DMA_TRNS_ERR | DMA_TRNS_CMPLT | DMA_HALF_TRNS)
                        );
...
void init_dma_usart(void) {
    ...
    dma_setup_transfer(USART_DMA_DEV, USART_TX_DMA_CHANNEL,
                        ...
                        (DMA_MINC_MODE | DMA_FROM_MEM | DMA_TRNS_ERR | DMA_TRNS_CMPLT));
...
void adc_dma_irq() {
     ...
     * Then, once the second half is ready, we'll process it.
     *
     ...
     * to do something useful. STM's example normalized the data.
     */

    dma_irq_cause irq_cause = dma_get_irq_cause(ADC_DMA_DEV, ADC_DMA_CHANNEL);

    uint8 complete_buffer = 1;

    /*
     * This case would only happen if this irq handler was called
     * directly by the user without any interrupts actually occuring.
     */
    if (irq_cause == DMA_NO_CAUSE)
        return;

    if (irq_cause == DMA_TRANSFER_ERROR) {
        /*
         * An error occured, which means we wrote to the
         * DMA Controller registers while it was enabled.
         * We'll handle this by setting a flag for loop.
         */
    ...
    for (uint16 i = (HALF_BUF_SIZE) * complete_buffer; i < (HALF_BUF_SIZE * (1 + complete_buffer)); i++) {
    ...
    if (irq_cause == DMA_TRANSFER_COMPLETE) {
        ...
        /*
         * We're done with the ADC channel for now. Disable it.
         */
        dma_disable(ADC_DMA_DEV, ADC_DMA_CHANNEL);

        // We may want to stop the ADC here for now.
...
void usart_tx_dma_irq() {
    ...
    dma_irq_cause irq_cause = dma_get_irq_cause(USART_DMA_DEV, USART_TX_DMA_CHANNEL);
